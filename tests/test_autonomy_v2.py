...
def write_test_repair_contract(
    ...
    *,
    ...
    contract.update(
        {
            ...
            "task_id": task_id,
            "title": "Repair the exact reviewer finding",
            "objective": "Repair the bounded finding and preserve the immutable review gate.",
            "dependencies": [],
            "role": "builder",
    ...
    contract.pop("lifecycle_stage", None)
    artifacts.write(
...
def test_replan_reclaims_unused_superseded_implementation_reservation(
    ...
    try:
        ...
        first_plan["nodes"][0]["task_contract"]["lifecycle_stage"] = "implementation-slice"
        persist_and_ingest_plan(
            config,
            state,
            first_plan,
        ...
        replacement["nodes"][0]["task_contract"]["lifecycle_stage"] = "implementation-slice"
        ...
        final_replacement["nodes"][0]["task_contract"]["lifecycle_stage"] = "implementation-slice"
        assert persist_and_ingest_plan(
            config,
            state,
            final_replacement,
...
def test_external_target_scope_and_recovery_slot_compile_guards() -> None:
    def implementation(key: str, scope: list[str]) -> dict[str, Any]:
        return {
            ...
            "objective": "Produce fresh target-tests evidence.",
            "scope": scope,
            "depends_on": [],
            "goal_ids": ["service"],
    ...
    assert {
        ...
        "toolchain.make",
    }.issubset(contract["required_capabilities"])

    proposal.update(
        proposal_kind="replan_delta",
        product_id="P-BUDGETED-REPLAN",
        parent_plan_id="PLAN-1",
        source_failure_id="failure-1",
        nodes=[implementation(f"repair-{index}", [f"src/repair_{index}.py"]) for index in range(4)],
    )
    with pytest.raises(
        PlanContractViolation,
...
def test_replanner_can_use_two_post_arbiter_builder_execution_slots(
    ...
    try:
        ...
        context = CompileContext(
            ...
            external_repository=True,
            proposal_artifact_ref="evidence/replan.json",
            remaining_recovery_execution_slots=int(budget["remaining_execution_slots"]),
        )
        ...
        with state._connection:
            assert (
                governor.reserve_execution_slots(
                    product_id="product-autonomy",
                    root_problem_signature=signature,
                    count=2,
                    progress=governor.progress_vector("product-autonomy"),
...
def test_AUT_P0_002_new_product_bootstrap_is_durable_and_idempotent(
    ...
    try:
        ...
        intent = state._connection.execute(
            "SELECT intent_id,status FROM side_effect_intents WHERE product_id=?",
            ("product-autonomy",),
        ).fetchone()
        assert intent is not None
        assert str(intent["status"]) == "VERIFIED"
        assert (
            state._connection.execute(
                "SELECT COUNT(*) FROM side_effect_receipts WHERE intent_id=?",
...
def failed_two_node_graph(
    ...
    *,
    ...
    artifacts = ArtifactStore(config)
    create_v2_product(state)
    for product_status in ("RISK_CLASSIFIED", "ARCHITECTED", "IMPLEMENTING"):
        state.transition_product("product-autonomy", product_status)
...
def test_accepted_repair_closes_transitive_chain_without_spending_budget(
    tmp_path: Path, reviewer: bool
) -> None:
    ...
    try:
        ...
        state.commit_task_outcome(
            TaskOutcome(
                task_id="T-SECOND-REPAIR",
                worker_id="transitive-repair-worker",
                lease_token=str(claimed["lease_token"]),
                expected_plan_revision=1,
                status="ACCEPTED",
...
def test_transitive_repair_chain_boundary_failure_is_atomic(
        ...
        with pytest.raises(ValueError, match=message):
            state.commit_task_outcome(
                TaskOutcome(
                    ...
                    lease_token=str(claimed["lease_token"]),
                    expected_plan_revision=1,
                    idempotency_key=sha256_text(f"boundary:{boundary}"),
                    status="ACCEPTED",
                    result_ref="internal://boundary",
                    result_digest=sha256_text("boundary"),
                )
            )
...
def test_repair_does_not_inherit_toolchain_capabilities_from_failed_node_lineage(
    tmp_path: Path,
) -> None:
    ...
    try:
        ...
        assert required == sorted(CAPABILITY_PROFILES["builder_workspace"])
        contract = json.loads(
...
def test_actionable_failure_from_readonly_reviewer_routes_to_replanner(
    ...
    try:
        ...
        assert tuple(decision) == ("RECOMPILE_AFFECTED_SUBGRAPH", "APPLIED")
    finally:
        state.close()
...
def test_cross_role_supersession_is_rejected_before_insert(tmp_path: Path) -> None:
    ...
    try:
        ...
        tasks_before = tuple(task["task_id"] for task in state.list_tasks("product-autonomy"))
        ...
        with pytest.raises(PlanContractViolation) as captured:
            FailureRouter(config, state, artifacts)._write_contract(
                ...
                failure=failure,
                hypothesis_id=(
                    str(source["hypothesis_id"]) if source.get("hypothesis_id") else None
                ),
                role="path-arbiter",
        ...
        assert (
            tuple(task["task_id"] for task in state.list_tasks("product-autonomy")) == tasks_before
        )
...
def test_reviewer_gate_failure_after_arbiter_uses_remaining_builder_slot(
    ...
    try:
        ...
        reviewer_schema = (
            "review-result.schema.json" if architecture else "security-review-result.schema.json"
        )
        contract.update(
            {
                "role": reviewer_role,
        ...
        expected_paths = (
            ["artifacts/**"] if architecture else ["pyproject.toml", "src/**", "tests/**"]
        )
    ...
    try:
        ...
        with state._lock, state._connection:
            ...
            state.transition_product(
                ...
                terminal_evidence_ref="internal://container-scan-review",
            )
        ...
        assert "toolchain.scanners" not in required
        repair_contract = json.loads(
            (config.evidence_dir / Path(str(repair["contract_ref"])).name).read_text(
                encoding="utf-8"
            )
...
def test_path_governor_live_router_enforces_one_arbiter_two_executions(
    ...
    """A fresh mandatory gate receives an independent structural budget."""
    ...
    try:
        ...
        tasks_before = len(state.list_tasks("product-autonomy"))
        next_repair_id = router.route(committed.failure_id)
        next_repair = state.get_task(next_repair_id)
        assert next_repair is not None
        assert next_repair_id != "T-PATH-LIVE-A"
        assert next_repair["role"] == "builder"
        assert next_repair["root_problem_signature"] != signature
        assert len(state.list_tasks("product-autonomy")) == tasks_before + 1

        product = state.get_product("product-autonomy")
        assert product is not None
        assert product["status"] == "IMPLEMENTING"
        prior_budget = state._connection.execute(
            """SELECT arbiter_calls_used, execution_attempts_used, status
                 FROM problem_budgets
                WHERE product_id=? AND root_problem_signature=?""",
            ("product-autonomy", signature),
        ...
        assert tuple(prior_budget) == (1, 2, "ACTIVE")
        next_budget = state._connection.execute(
            """SELECT arbiter_calls_used, execution_attempts_used, status
                 FROM problem_budgets
                WHERE product_id=? AND root_problem_signature=?""",
            ("product-autonomy", next_repair["root_problem_signature"]),
        ).fetchone()
        assert next_budget is not None
        assert tuple(next_budget) == (0, 1, "ACTIVE")
    finally:
        state.close()
...
def test_path_arbiter_worker_runs_readonly_and_creates_one_replanner(
    ...
    try:
        ...
        assert result is not None and result.status == "completed", (
            ...
            result.detail if result is not None else None,
        )
...
def test_reviewer_revalidation_lineage_recovery_repairs_existing_shadow_state(
    ...
    try:
        ...
        source_contract_path = config.evidence_dir / Path(str(failed["contract_ref"])).name
...
def test_AUT_P0_006_missing_plan_output_schema_quarantines_controller_defect(
    tmp_path: Path,
) -> None:
    config, state, artifacts, failure_id, _ = failed_two_node_graph(tmp_path)
...
def test_AUT_P0_006_legacy_contract_ref_is_quarantined_without_reconstruction(
    tmp_path: Path,
) -> None:
    config, state, artifacts, failure_id, _ = failed_two_node_graph(
        tmp_path,
    ...
    try:
        ...
        assert routed_id == ""
        assert state.get_product("product-autonomy")["terminal_reason"] == "invalid_task_contract"
        reconstructed = [
            event
            for event in state.events("product-autonomy")
...
def test_AUT_P0_006_missing_contract_is_quarantined_without_permissive_scope(
    tmp_path: Path,
) -> None:
    config, state, artifacts, failure_id, _ = failed_two_node_graph(
        tmp_path,
    ...
    try:
        ...
        incident = state._connection.execute(
            ...
            SELECT reason_code, status, evidence_ref
             ...
             WHERE product_id=?
               AND reason_code='invalid_task_contract'
            """,
            ("product-autonomy",),
        ).fetchone()
        assert incident is not None
        assert incident["status"] == "OPEN"
        assert str(incident["evidence_ref"]).startswith("internal://contract-integrity/")
    finally:
        state.close()
...
def test_diagnosis_reassessment_uses_one_arbiter_then_fails_safe(
    ...
    try:
        ...
        assert committed.failure_id is not None
        tasks_before = len(state.list_tasks("product-autonomy"))
        reassessment_id = router.route(committed.failure_id)
        reassessment = state.get_task(reassessment_id)
        assert reassessment is not None
        assert reassessment_id != diagnosis_task_id
        assert reassessment["role"] == "path-arbiter"
        assert reassessment["root_problem_signature"] != diagnosis["root_problem_signature"]
        assert len(state.list_tasks("product-autonomy")) == tasks_before + 1
        assert state.get_product("product-autonomy")["status"] == "IMPLEMENTING"
        diagnosis_hypothesis = next(
            item
            for item in state.list_hypotheses("product-autonomy")
            if item["hypothesis_id"] == diagnosis_hypothesis_id
        )
        assert diagnosis_hypothesis["status"] == "ACTIVE"
    finally:
        state.close()
...
def test_AUT_P0_019_invalid_output_schema_migration_requires_new_release_epoch(
    tmp_path: Path,
) -> None:
    ...
    try:
        ...
        assert routed_id == ""
        product = restarted.get_product("product-autonomy")
        assert product is not None
        assert product["status"] == "FAILED_SAFE"
        assert product["terminal_reason"] == "controller_exception_file_not_found_error"
    finally:
...
def test_AUT_P0_019_exhausted_graph_quarantines_controller_liveness_defect(
    tmp_path: Path,
) -> None:
    config = configured(tmp_path)
    state = StateStore(config.database_path)
    try:
        ...
        assert terminal_product["terminal_reason"] == "liveness_invariant_violation"
...
def test_legacy_replanner_scope_contract_is_quarantined_without_correction_task(
    tmp_path: Path,
) -> None:
    ...
    try:
        ...
        assert product is not None
        assert product["status"] == "FAILED_SAFE"
        assert product["terminal_reason"] == "invalid_capability_contract"
        incident = state._connection.execute(
            """SELECT reason_code, status FROM controller_incidents
        ...
        assert incident is not None
        assert incident["reason_code"] == "invalid_capability_contract"
        assert incident["status"] == "OPEN"
    finally:
        state.close()
