...
@pytest.mark.parametrize(
    "unit_name",
    ["hermes-factory-worker.service", "hermes-factory-worker-2.service"],
)
def test_permanent_worker_units_enforce_attested_sandbox(unit_name: str) -> None:
    unit = (Path("config/systemd") / unit_name).read_text(encoding="utf-8")
    for setting in (
    ...
    assert "/opt/hermes-codex-runtime" not in unit
