...
def path_governor_execution_budget(
    ...
    *,
    ...
    raw_remaining = max(0, PATH_GOVERNOR_EXECUTION_SLOT_LIMIT - used) if status == "ACTIVE" else 0
...
class AutonomyStore:
    ...
    def _ingest_plan(
        ...
        *,
        ...
        connection.execute(
            """INSERT INTO plans
               (plan_id, product_id, revision, parent_plan_id, source_failure_id,
                ...
                compiler_version, lifecycle_version, proposal_artifact_ref,
                proposal_digest)
               VALUES (?, ?, ?, ?, ?, 'ACTIVE', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                plan_id,
                ...
                now,
                now,
                plan.get("compiler_version"),
                plan.get("lifecycle_version"),
                plan.get("proposal_artifact_ref"),
                plan.get("proposal_digest"),
            ),
        )
        ...
        for rank, node in enumerate(plan["nodes"]):
            ...
            contract = dict(node["task_contract"])
            contracts_by_node[node_id] = contract
            task_id = str(
            ...
            connection.execute(
                ...
                (
                    ...
                    contract.get("lifecycle_stage"),
                    contract.get("review_kind"),
                    contract.get("evidence_profile"),
                    stable_json(contract.get("consumes_evidence_types", [])),
                    stable_json(contract.get("produces_evidence_types", [])),
                    stable_json(contract.get("completion_obligation_ids", [])),
            ...
            if creator_root_problem_signature and execution_slot_cost(contract) == 1:
        ...
        new_task_ids = set(node_to_task.values())
        for node_id, contract in contracts_by_node.items():
            for dependency_value in contract.get("dependencies", []):
                dependency_id = str(dependency_value)
                if dependency_id in new_task_ids:
                    raise ValueError(
                        "compiled task dependencies must use plan edges for internal nodes"
                    )
                dependency = connection.execute(
                    "SELECT product_id FROM tasks WHERE task_id=?",
                    (dependency_id,),
                ).fetchone()
                ...
                connection.execute(
                    """INSERT OR IGNORE INTO task_edges
                       (plan_id, from_task_id, to_task_id, edge_type,
                        required, created_at)
                       VALUES (?, ?, ?, 'evidence_from', 1, ?)""",
                    (
                        plan_id,
                        dependency_id,
                        node_to_task[node_id],
                        now,
                    ),
                )
    ...
    def _insert_successor(
        ...
        if supersedes_task_id:
            ...
            if source is None or not supersession_is_compatible(dict(source), replacement):
                raise PlanContractViolation(
                    "cross-role supersession is incompatible with the source task contract",
                    reason_code="cross_role_supersession_invalid",
                )
    ...
    def _supersession_chain(
        ...
        while supersedes_task_id:
            ...
            repair_ancestor = str(ancestor["stage_key"] or "") == "repair" and not reviewer_ancestor
        ...
        with self.lock:
            ...
            try:
                ...
                if outcome.accepted_result_ref is not None:
                    ...
                    if retired_architecture_binding is not None:
                        inject("after_architecture_result_bind")
                        governor.advance_reviewed_architecture_evidence_for_correction(
                            outcome.task_id,
                            retired_architecture_binding,
                        )
                        inject("after_architecture_evidence_edge_update")
                    ...
                    target = self.connection.execute(
                        """SELECT task_id FROM tasks
                           WHERE product_id=? AND plan_id=?
                             AND lifecycle_stage='observation'
                             AND graph_status NOT IN
                                 ('ACCEPTED','SUPERSEDED','REJECTED','CANCELLED')""",
                        (
                            str(task["product_id"]),
                    ...
                    self.connection.execute(
                        """UPDATE tasks
                           SET required_predecessor_digest=?, available_at=?,
                               graph_status='WAITING_TIME', status='WAITING',
                               updated_at=?
                           WHERE task_id=?""",
                        (digest, available_at, now, str(target[0][0])),
                    )
                ...
                if outcome.status in TERMINAL_SUCCESS and failure_id is None:
                    ...
                    architecture_correction = bool(
                        str(task["role"] or "").replace("_", "-") == "solution-architect"
                        and str(task["stage_key"] or "") == "repair"
                        and str(task["capability_profile"] or "") == "planning_readonly"
                        and str(task["output_schema"] or "") == "architecture-package.schema.json"
                    )
                    ...
                    if supersedes_task_id:
                        ...
                        chain_task_ids = [
                            str(ancestor["task_id"]) for ancestor in supersession_chain
                        ]
                        reviewer_task = (
                            ...
                            and str(task["stage_key"] or "") == "repair"
                            and str(supersession_chain[-1]["role"] or "") in _REVIEW_PROFILE_ROLES
                            and str(supersession_chain[-1]["capability_profile"] or "")
                            == "reviewer_readonly"
                            else None
                        )
                        reviewer_revalidation = reviewer_task is not None
                        reviewer_task_id = (
                            str(reviewer_task["task_id"]) if reviewer_task is not None else None
                        )
                        superseded_task_ids = [
                            task_id for task_id in chain_task_ids if task_id != reviewer_task_id
                        ]
                        ...
                        if transitive_superseded_ids:
                            placeholders = ",".join("?" for _ in transitive_superseded_ids)
                            self.connection.execute(
                                f"""UPDATE tasks
                                   SET graph_status='SUPERSEDED', status='DONE',
                                       updated_at=?
                        ...
                        if resolved_sibling_ids or redundant_repair_ids or len(chain_task_ids) > 1:
                            self.state._record_event(
                                str(task["product_id"]),
                                outcome.task_id,
                                "redundant_repair_work_suppressed",
                                {
                                    ...
                                    "reviewer_revalidation_task_id": (reviewer_task_id),
                                },
                            )
