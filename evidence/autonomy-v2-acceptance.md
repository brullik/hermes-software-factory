...
| Semantic plan precommit validation | yes | PASS | Worker regression proves exact safe edge coordinates schedule bounded repair; identity regressions prove existing task keys and reused plan IDs with changed immutable digests are rejected before mutation; every task output schema must exist in the immutable release registry |
...
| In-place retry routing exclusivity | yes | PASS | Retry work remains in place while bounded; after terminal failure only the unresolved causal leaf may create recovery work, migration v7 removes historical ancestor/descendant duplicate branches, and missing planned output schemas route to Replanner while their obsolete controller incidents are resolved |
...
| Executable-plan liveness | yes | PASS | Planning-only graphs and plans referencing unregistered output schemas are rejected; only current-active-plan work is liveness proof; exhausted graphs create a claimable Replanner task, stranded routed recovery is re-anchored to the active revision, migration v8 repairs historical invalid-schema branches, and migration v9 closes their stale controller incidents |
...
| Full existing suite | yes | PASS | 206 tests; `.venv\Scripts\python.exe -m pytest -q --tb=short` |
