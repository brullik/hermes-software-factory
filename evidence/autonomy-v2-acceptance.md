...
| In-place retry routing exclusivity | yes | PASS | Retry work remains in place while bounded; after terminal failure only the unresolved causal leaf may create recovery work, while migration v7 removes historical ancestor/descendant duplicate branches |
...
| Full existing suite | yes | PASS | 203 tests; `.venv\Scripts\python.exe -m pytest -q --tb=short` |
